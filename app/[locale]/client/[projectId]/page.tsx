...
export default async function ClientPage({ params }: { params: Promise<{ projectId: string }> }) {
    ...
    try {
        ...
        // Base64 regex check roughly or just try-catch
        // Project IDs are usually short (12 chars or UUID)
        // Base64 configs are very long JSONs
        if (decodedProjectId.length > 50 || decodedProjectId.includes('{')) {
            const json = Buffer.from(decodedProjectId, 'base64').toString('utf-8');
            config = JSON.parse(json);
            // Map legacy whatsapp field to adminWhatsapp if needed
